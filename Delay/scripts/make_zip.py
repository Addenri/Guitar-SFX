...
def  main():
  ...
  installer = "\installer\Delay Installer.exe"
   
  if demo:
    installer = "\installer\Delay Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Delay manual.pdf" 
  ]
  ...
  ZIPNAME = "Delay-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Delay-v" + FULLVERSIONSTR + "-win-demo.zip"
