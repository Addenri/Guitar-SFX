...
if %1 == 1 (echo Making Delay Windows DEMO VERSION distribution ...) else (echo Making Delay Windows FULL VERSION distribution ...)
...
msbuild Delay.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Delay.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Delay.aaxplugin\Contents\Win32\Delay.aaxplugin --out .\build-win\aax\bin\Delay.aaxplugin\Contents\Win32\Delay.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Delay.aaxplugin\Contents\x64\Delay.aaxplugin --out .\build-win\aax\bin\Delay.aaxplugin\Contents\x64\Delay.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Delay.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Delay.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Delay Installer" ".\installer\Delay Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Delay Installer.exe" ".\installer\Delay Demo Installer.exe"
REM -del ".\installer\Delay Installer.exe"
REM -)
