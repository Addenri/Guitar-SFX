...
def  main():
  ...
  installer = "\installer\NoiseGate Installer.exe"
   
  if demo:
    installer = "\installer\NoiseGate Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\NoiseGate manual.pdf" 
  ]
  ...
  ZIPNAME = "NoiseGate-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "NoiseGate-v" + FULLVERSIONSTR + "-win-demo.zip"
