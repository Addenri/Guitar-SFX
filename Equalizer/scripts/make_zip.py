...
def  main():
  ...
  installer = "\installer\Equalizer Installer.exe"
   
  if demo:
    installer = "\installer\Equalizer Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Equalizer manual.pdf" 
  ]
  ...
  ZIPNAME = "Equalizer-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Equalizer-v" + FULLVERSIONSTR + "-win-demo.zip"
