...
def  main():
  ...
  installer = "\installer\Distortion Installer.exe"
   
  if demo:
    installer = "\installer\Distortion Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Distortion manual.pdf" 
  ]
  ...
  ZIPNAME = "Distortion-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Distortion-v" + FULLVERSIONSTR + "-win-demo.zip"
